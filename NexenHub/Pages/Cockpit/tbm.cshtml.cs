...
{
    public class tbmModel : PageModel
    {
        ...
        public void LoadPhase1()
        {
            ...
            foreach (DataRow row in database.GetMachineList(WC_ID: "T", FACT_ID: "NEX1").Rows)
            {
                Phase1.Add(new MachineBasicInfo(row["EQ_ID"].ToString()));    
            }

            foreach (DataRow row in database.GetMachineList(WC_ID: "T", FACT_ID: "NEX2").Rows)
            {
